...
async function generateSVG(fn: SvelteActionFunction, output: string) {
	...
	const data = {
		name: fn.name,
		package: `@svelteaction/${fn.package}`,
		line1: lines[0] || "",
		line2: lines[1] || "",
		line3: lines[2] || "",
	}
	...
	await Promise.all(
		names.map(async (file) => {
			...
			if (fn) {
				await generateSVG(
					fn,
					`packages/.vitepress/dist/og-${fn.name}.png`
				)
				html = html.replace(
					/svelteaction\.org\/og\.png/g,
					`svelteaction.org/og-${fn.name}.png`.toLocaleLowerCase()
				)
			}
